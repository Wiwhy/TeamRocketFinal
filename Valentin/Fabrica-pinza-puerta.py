...
# Seguros anti-spam para la cruceta de la puerta
...
try:
    while True:
        ...
        # ==========================================
        # LÓGICA DE LA PUERTA (CRUCETA / D-PAD)
        # ==========================================
        btn_abrir = False
        btn_cerrar = False
        
        try:
            # En Pygame, get_hat(0) devuelve una tupla (x, y)
            # y = 1 es Arriba, y = -1 es Abajo
        ...
        # Detectar pulsación de Cruceta Arriba (Abrir)
        if btn_abrir and not estado_abrir_ant:
            sock.sendto(b"PUERTA_ABRIR\n", (ROBOT_IP, PORT))
            print("📡 COMANDO ENVIADO: ¡Solicitando ABRIR barrera (Flecha Arriba)!")
            
        # Detectar pulsación de Cruceta Abajo (Cerrar)
        if btn_cerrar and not estado_cerrar_ant:
            sock.sendto(b"PUERTA_CERRAR\n", (ROBOT_IP, PORT))
            print("📡 COMANDO ENVIADO: ¡Solicitando CERRAR barrera (Flecha Abajo)!")
        ...
        # Enviamos paquete de telemetría constante
