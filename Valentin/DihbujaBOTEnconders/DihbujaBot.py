...
def enviar_configuracion():
    try:
        ...
        # 2. AHORA SON TICKS POR CM (Calibración de Encoders)
        tk_f_tri = float(entry_tk_f_tri.get())
        tk_diag = float(entry_tk_diag.get())
        tk_f_cr = float(entry_tk_f_cr.get())
        tk_lat = float(entry_tk_lat.get())
        ...
        # COMPRESIÓN DEL PAQUETE UDP
        mensaje = (f"C,{figura},{tk_f_tri},{tk_diag},{tk_f_cr},{tk_lat},"
        ...
        lbl_estado.config(text="✅ Odometría Sincronizada", fg="green")
    except ValueError:
        messagebox.showerror("Error", "Revisa los campos numéricos.")

def iniciar_trazado():
    sock.sendto("G\n".encode('utf-8'), (ROBOT_IP, PORT))
    lbl_estado.config(text="▶️ Ejecutando dibujo con ENCODERS...", fg="blue")
...
# --- INTERFAZ GRÁFICA ---
root = tk.Tk()
root.title("Mecanum Matrix - Odometría por Encoders")
...
# 1. Ticks en lugar de ms
f_física = tk.LabelFrame(root, text="1. Calibración Encoders (Ticks de lectura por cada 1 CM real)", fg="blue", font=("Arial", 9, "bold"))
f_física.pack(fill="x", pady=3)
tk.Label(f_física, text="Triáng. Recto:").grid(row=0, column=0, padx=2); entry_tk_f_tri = tk.Entry(f_física, width=6, justify="center"); entry_tk_f_tri.insert(0, "40.0"); entry_tk_f_tri.grid(row=0, column=1, padx=5)
tk.Label(f_física, text="Triáng. Diag:").grid(row=0, column=2, padx=2); entry_tk_diag = tk.Entry(f_física, width=6, justify="center"); entry_tk_diag.insert(0, "55.0"); entry_tk_diag.grid(row=0, column=3, padx=5)
tk.Label(f_física, text="Cuad/Rect Recto:").grid(row=0, column=4, padx=2); entry_tk_f_cr = tk.Entry(f_física, width=6, justify="center"); entry_tk_f_cr.insert(0, "40.0"); entry_tk_f_cr.grid(row=0, column=5, padx=5)
tk.Label(f_física, text="Cuad/Rect Lateral:").grid(row=0, column=6, padx=2); entry_tk_lat = tk.Entry(f_física, width=6, justify="center"); entry_tk_lat.insert(0, "65.0"); entry_tk_lat.grid(row=0, column=7, padx=5)
...
# Paneles 2, 3 y 4 (Se mantienen igual)
