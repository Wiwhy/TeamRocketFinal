// Version 3.8.9 (Dual Mode + Freno Activo + Ultrasonido Trasero + Contragiro Solo Atrás)
...
const int VEL_INICIO_RETRO     = 210;  

const int VEL_INICIO_AVANCE    = 210;

const int VEL_ATAQUE           = 210;

const int VEL_RETROCESO        = 210;
...
// 4. CONFIGURACIONES ESPECÍFICAS
  ...
  int  TIEMPO_INICIO_AVANCE = 700;

  int  TIEMPO_RETROCESO     = 700;

  int  VEL_BUSQUEDA         = 200;
  ...
  int  TIEMPO_INICIO_AVANCE = 1470;

  int  TIEMPO_RETROCESO     = 1470;

  int  VEL_BUSQUEDA         = 150;
  ...
  switch (estado) {
    ...
    case BUSCAR:
      ...
      if (lineaFrente) {
        ...
        retroceder(); // Contramarcha recta (sin giro)
      ...
      if (lineaAtras) {
        ...
        avanzar(); // <-- MODIFICADO: Freno en seco recto

        velocidad(255);

        delay(TIEMPO_CONTRAMARCHA_RECTO); // <-- MODIFICADO: Usa el tiempo recto
